...
export default function Assinatura() {
  ...
  //     setMessage("Pagamento realizado com sucesso! Sua assinatura está ativa.");
  //     setStatus("ATIVO");
  ...
  const handleGerarPix = async (plano: string) => {
    ...
    try {
      ...
      setQrCode(`data:image/png;base64,${data.result.point_of_interaction.transaction_data.qr_code_base64}`);
    } catch (error) {
  ...
  return (
    <div className="mx-auto p-6">
      ...
      {message && <p className={`text-${status === "ATIVO" ? "green" : "red"}-600`}>{message}</p>}

      <div className="flex gap-12">
      <div className="grid gap-4 w-[60%]">
        <div className="border p-4 rounded-lg shadow-md">
          <h2 className="text-lg font-bold">Plano Gratuito</h2>
          <p className="text-gray-600">Teste grátis com funcionalidades limitadas.</p>
          <p className="text-green-600 font-bold">Status: ATIVO</p>
          <Button disabled className="w-full bg-gray-400 text-white mt-2">Plano Ativo</Button>
        </div>
        ...
        <div className="border p-4 rounded-lg shadow-md w-40%">
          ...
          {status !== "INTERMEDIARIO" && (
            <Button onClick={() => handleGerarPix("INTERMEDIARIO")} disabled={loading} className="bg-blue-600 text-white w-full mt-2">
        ...
        <div className="border p-4 rounded-lg shadow-md">
          ...
          {status !== "ILIMITADO" && (
            <Button onClick={() => handleGerarPix("ILIMITADO")} disabled={loading} className="bg-green-600 text-white w-full mt-2">
              {loading ? "Gerando PIX..." : "Pagar via PIX"}
            </Button>
          )}
        </div>
      ...
      {pixCode && (
        <div className="p-4 border rounded-lg shadow-md bg-gray-100">
          ...
          <Image src={qrcode} alt="QR Code PIX" width={480} height={480 } className="mx-auto my-4" />
          <textarea className="w-full p-2 border rounded-md bg-white" value={pixCode} readOnly />
          <Button onClick={() => navigator.clipboard.writeText(pixCode)} className="w-full bg-gray-700 text-white mt-2">
            Copiar Código PIX
          </Button>
